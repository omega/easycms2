...
#__PACKAGE__->mk_accessors(qw/pre_process post_process/);
...
#    $self->post_process($config->{PRE_PROCESS});
...
#    $config->{PRE_PROCESS} = [ sub { $copy->pre_process } ];
#    $config->{POST_PROCESS} = [ sub { $copy->post_process } ];
